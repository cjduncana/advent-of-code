...
type Tower
    = TowerComplete String Int (List Tower)
    | TowerIncomplete String Int (List String)


solution : String
solution =
    case Result.map (flip buildTower Dict.empty) input |> Result.andThen identity of
        Ok tower ->
            getName tower
...
buildTower : Nonempty Tower -> Dict String Tower -> Result String Tower
buildTower (Nonempty tower towers) record =
    case towers of
        [] ->
            if isTowerComplete tower then
                Ok tower
            else if Dict.isEmpty record then
                Err "A complete tower was not found"
            else
                let
                    ( _, newTower ) =
                        addTowers record tower
                in
                    if isTowerComplete newTower then
                        Ok tower
                    else
                        Err "A complete tower was not found"

        t :: ts ->
            if List.all isTowerComplete (tower :: towers) && Dict.isEmpty record then
                Err "More than one complete tower found"
            else if isTowerComplete tower then
                Dict.insert (getName tower) tower record
                    |> buildTower (Nonempty t ts)
            else
                let
                    ( newRecord, newTower ) =
                        addTowers record tower
                in
                    if isTowerComplete newTower then
                        Dict.insert (getName newTower) newTower newRecord
                            |> buildTower (Nonempty t ts)
                    else
                        buildTower (Nonempty t (ts ++ [ newTower ])) newRecord


addTowers : Dict String Tower -> Tower -> ( Dict String Tower, Tower )
addTowers towers tower =
    case tower of
        TowerComplete _ _ _ ->
            ( towers, tower )

        TowerIncomplete name weight towerNames ->
            towerNames
                |> List.map (flip Dict.get towers)
                |> List.foldl (Maybe.map2 (::)) (Just [])
                |> Maybe.map
                    (TowerComplete name weight
                        >> (,) (List.foldl Dict.remove towers towerNames)
                    )
                |> Maybe.withDefault ( towers, tower )


getName : Tower -> String
getName tower =
    case tower of
        TowerComplete name _ _ ->
            name

        TowerIncomplete name _ _ ->
            name


isTowerComplete : Tower -> Bool
isTowerComplete tower =
    case tower of
        TowerComplete _ _ _ ->
            True

        TowerIncomplete _ _ _ ->
            False


towersDecoder : Decoder (Nonempty Tower)
towersDecoder =
    Decode.list towerDecoder
        |> Decode.andThen
            (Nonempty.fromList
                >> Maybe.map Decode.succeed
                >> Maybe.withDefault (Decode.fail "Must have at least one tower")
            )


towerDecoder : Decoder Tower
towerDecoder =
    Decode.string
        |> Decode.andThen
            (\info ->
                case String.split " -> " info of
                    [ tower, towers ] ->
                        towerInfoDecoder tower
                            |> Decode.map
                                (\( name, weight ) ->
                                    String.split ", " towers
                                        |> TowerIncomplete name weight
                                )
                    ...
                    _ ->
                        Decode.fail "Not a valid tower"
            )


towerInfoDecoder : String -> Decoder ( String, Int )
towerInfoDecoder info =
    case String.split " " info of
        [ name, weight ] ->
        ...
        _ ->
            Decode.fail "Not a valid tower information"


weightDecoder : String -> Decoder Int
weightDecoder =
    String.dropLeft 1
        >> String.dropRight 1
        >> String.toInt
        >> Result.map Decode.succeed
        >> Result.withDefault (Decode.fail "Not a valid weight")
