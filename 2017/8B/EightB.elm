module EightB exposing (solution)
...
type Comparison
    = Comparison String (Int -> Bool)


solution : Int
solution =
    followInstuctions input
        |> Dict.foldl (\_ -> Tuple.second >> max) Random.minInt


followInstuctions : List Instruction -> Registers
followInstuctions =
    List.foldl followInstuction Dict.empty


followInstuction : Instruction -> Registers -> Registers
followInstuction { command, comparison } registers =
    if compareRegister comparison registers then
        modifyRegisters command registers
    else
        registers


compareRegister : Comparison -> Registers -> Bool
compareRegister (Comparison register comparison) =
    getValue register >> Tuple.first >> comparison


modifyRegisters : Command -> Registers -> Registers
modifyRegisters (Command register change) registers =
    getValue register registers
        |> Tuple.mapFirst change
        |> (\( newValue, maxValue ) -> ( newValue, max newValue maxValue ))
        |> flip (Dict.insert register) registers
...
input =
    rawInput
        ...
        |> Decode.decodeString (Decode.list instructionDecoder)
        |> Result.withDefault []
...
instructionDecoder =
    Decode.string
        |> Decode.andThen
            (\input ->
                case String.split " if " input of
                    [ command, comparison ] ->
                        comparisonDecoder comparison
                            |> Decode.map2 Instruction (commandDecoder command)

                    _ ->
                        Decode.fail "Not a valid instruction"
            )


commandDecoder : String -> Decoder Command
commandDecoder input =
    case String.split " " input of
        [ register, direction, amount ] ->
            directionDecoder direction
                |> Decode.map flip
                |> andMap (intDecoder amount)
                |> Decode.map (Command register)

        _ ->
            Decode.fail "Not a valid command"


comparisonDecoder : String -> Decoder Comparison
comparisonDecoder input =
    case String.split " " input of
        [ register, conditional, amount ] ->
            conditionalDecoder conditional
                |> Decode.map flip
                |> andMap (intDecoder amount)
                |> Decode.map (Comparison register)

        _ ->
            Decode.fail "Not a valid comparison"


directionDecoder : String -> Decoder (Int -> Int -> Int)
directionDecoder input =
    case input of
        "inc" ->
            Decode.succeed (+)
        ...
        _ ->
            Decode.fail "Not a valid direction"


conditionalDecoder : String -> Decoder (Int -> Int -> Bool)
conditionalDecoder input =
    case input of
        "==" ->
            Decode.succeed (==)
        ...
        _ ->
            Decode.fail "Not a valid conditional"
...
getValue : String -> Registers -> ( Int, Int )
getValue register =
    Dict.get register >> Maybe.withDefault ( 0, 0 )
