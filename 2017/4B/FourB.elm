module FourB exposing (solution)
...
solution =
    input
        |> List.map (isValid [] >> count)
        |> List.sum


isValid : List Anagram -> List String -> Bool
isValid previous words =
    case words of
        [] ->
            True
        ...
        w :: ws ->
            let
                anagram =
                    anagramMaker w
            in
                if List.member anagram previous then
                    False
                else
                    isValid (anagram :: previous) ws


anagramMaker : String -> Anagram
anagramMaker =
    String.split ""
        >> List.foldr countString Dict.empty


countString : String -> Anagram -> Anagram
countString char anagram =
    Dict.get char anagram
        |> Maybe.map ((+) 1 >> flip (Dict.insert char) anagram)
        |> Maybe.withDefault (Dict.insert char 1 anagram)
